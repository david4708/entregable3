...
const ResidentList = ({ location }) => {
  ...
  {
    arrayPages.map(page=><li onClick={()=>setCurrentPage(page)} className={`text-white font-bold p-3 cursor-pointer rounded-md ${page === currentPage && "bg-green-800 text-white font-bold"}`} key={page}>{page}</li>)
  }
